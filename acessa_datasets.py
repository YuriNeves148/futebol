...
portugal_2423_df = pd.read_csv('datasets/noruega_2324.csv')
portugal_2423_df = portugal_2423_df[['Date','HomeTeam','AwayTeam','FTHG','FTAG','FTR','HTHG','HTAG','HTR','HS','AS','HST','AST','HF','AF','HC','AC','HY','AY','HR','AR']]
portugal_2526_df['Temporada'] = "25/26"
portugal_2425_df['Temporada'] = "24/25"
portugal_2423_df['Temporada'] = "23/24"

argentina_tudo = pd.read_csv('datasets/argentina.csv')
argentina_tudo['Date'] = pd.to_datetime(argentina_tudo['Date'], format='%d/%m/%Y')
argentina_tudo['Date'] = argentina_tudo['Date'].dt.strftime('%d/%m/%Y')
...
brasil_tudo['Date'] = brasil_tudo['Date'].dt.strftime('%d/%m/%Y')
...
portugal_df = pd.concat([portugal_2526_df, portugal_2425_df, portugal_2423_df])
